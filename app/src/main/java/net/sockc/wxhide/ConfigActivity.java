...
public class ConfigActivity extends Activity {
    ...
    private void buildUi() {
        ...
        title.setText("WX Hide LSP 设置 v0.2.5");
        ...
        tips.setText("v0.2.5：配置页主要从微信 设置 → 功能 → WX Hide LSP 打开。\n"
                + "保存规则后会通知微信进程重新读取配置，通常不需要重启微信；如果当前页面没有立刻刷新，返回/滑动/重新搜索一下即可。\n"
                + "只隐藏命中的联系人行和聊天记录行；搜索隐藏联系人时会同步清掉“联系人”标题、灰色空白和“加载中”残留，不隐藏网络搜索。");
        tips.setTextSize(14);
        ...
        note.setText("说明：\n"
                ...
                + "5. 修改规则后通常不用重启微信；未刷新时返回/滑动/重新搜索一下。\n"
    ...
    private void saveLocalOnly() {
        ...
        updateConfigText(enabled, rules, deep, entry, launcher);
        notifyWechatConfigChanged();
        Toast.makeText(this, "已保存；已通知微信刷新配置，通常不需要重启微信", Toast.LENGTH_LONG).show();
    }
    ...
    private void writeGlobalConfig() {
        ...
        updateConfigText(enabled, rules, deep, entry, launcher);
        notifyWechatConfigChanged();
        Toast.makeText(this, globalOk ? "已写入全局备用配置，并通知微信刷新" : "全局配置写入失败：未授权 ROOT 或 su 不可用；如果已经 hit，可忽略", Toast.LENGTH_LONG).show();
    }
    ...
    private void notifyWechatConfigChanged() {
        try {
            Intent i = new Intent(Prefs.ACTION_CONFIG_CHANGED);
            i.setPackage(Prefs.WECHAT_PACKAGE);
            i.putExtra("from", Prefs.PACKAGE_NAME);
            sendBroadcast(i);
        } catch (Throwable ignored) {}
        try {
            Intent i = new Intent(Prefs.ACTION_CONFIG_CHANGED);
            i.setPackage(Prefs.PACKAGE_NAME);
            sendBroadcast(i);
        } catch (Throwable ignored) {}
    }
